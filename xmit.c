...
#include <sys/types.h>/* $Revision: 1.2 $ */
#include "ccgi.h"     /* Compile with:
...
{CGI_varlist *vl; int tlen=strlen(TMP_PATH); FILE *log; const char *name;
 ...
 {FILE *fp; CGI_value *val; struct stat junk_buf; int i, j;
  ...
  for(i = 0; val[i]; i++) /* Does filename match TMP_PATH, and exist?  */
   ...
   {time_t epoch = time(NULL); struct tm *rightnow = localtime(&epoch);

    j = i++; /* Now, val[j] == tmp_name, val[i] == user's sent name.   */
    strftime(dst, BUFSIZ, "%y/%m/%d %H:%M:%S", rightnow);
    fprintf(log, "%s %s %s", dst, getenv("REMOTE_ADDR"), val[i]);

    if(!strlen(val[i]) || strchr(val[i], '/') || strchr(val[i], '\\') ||
     strlcpy(dst, prefix, BUFSIZ) >= BUFSIZ || /* Skip if empty, over- */
     strlcat(dst, val[i], BUFSIZ) >= BUFSIZ)   /*  size, or malicious. */
    {
     printf("error\n"); fprintf(log, " _ABUSE-RETAINED_ %s\n", val[j]);
     continue;
    }

    if(link(val[j], dst))
    { /* On link failure, try our best to keep this data.              */
     if(strlcat(dst, val[j] + tlen, BUFSIZ) >= BUFSIZ ||
